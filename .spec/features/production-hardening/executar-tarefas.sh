#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano production-hardening` em 2026-08-29 22:16
...
RUN_ID='master-event-driven-microservices-architecture-patterns-production-hardening-mtexx6kx'
...
preparar_ambiente() {
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/master-event-driven-microservices-architecture-patterns-production-hardening-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/master-event-driven-microservices-architecture-patterns-production-hardening"
...
T-001 — "Criar gates estruturais de segurança e operação"
  critérios/refs: AC-001 (Credenciais locais não entram no Git), AC-002 (Configurações usam variáveis de ambiente), AC-003 (Maven Wrapper completo), AC-004 (Imagens Docker exigem ação explícita), AC-005 (Config Server usa o repositório local por padrão), AC-006 (Operações criptográficas exigem autenticação), AC-007 (Gateway exige token nas rotas de negócio), AC-008 (API reativa exige token nas consultas), AC-009 (Docker Compose validável sem segredos reais), AC-010 (Compose sem campos obsoletos), AC-011 (Guia de execução completo), AC-012 (CI verifica segurança, contrato e build)
  arquivos permitidos (e seus testes): tests/production-hardening.test.mjs, .spec/constituicao.md, onpspec.config.json
  mensagem de commit: "T-001 production-hardening: Criar gates estruturais de segurança e operação"

Regras inegociáveis:
- Todo critério de aceite referenciado vira teste com @spec:AC-xxx no título.
- NUNCA enfraqueça, pule (skip/todo) ou apague um teste para passar — teste pulado não é prova e o audit acusa.
- Rode os testes localmente com `node --test tests` até passarem.
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high &&
    rodar_tarefa 'faixa-2' 'T-004' 'Você executa UMA tarefa da feature "production-hardening" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-hardening/spec.md, .spec/features/production-hardening/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "Tornar o Config Server local e seguro por padrão"
  critérios/refs: AC-005 (Config Server usa o repositório local por padrão), AC-006 (Operações criptográficas exigem autenticação), AC-009 (Docker Compose validável sem segredos reais)
  arquivos permitidos (e seus testes): config-server/src/main/resources/application.yml, config-server/src/main/resources/application-git.yml, config-server/src/main/java/com/microservices/demo/config/server/config/SecurityConfig.java, docker-compose/services.yml, docker-compose/config_server.yml
  mensagem de commit: "T-004 production-hardening: Tornar o Config Server local e seguro por padrão"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high &&
    rodar_tarefa 'faixa-2' 'T-006' 'Você executa UMA tarefa da feature "production-hardening" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-hardening/spec.md, .spec/features/production-hardening/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-006 — "Modernizar e validar a composição Docker"
  critérios/refs: AC-009 (Docker Compose validável sem segredos reais), AC-010 (Compose sem campos obsoletos)
  arquivos permitidos (e seus testes): docker-compose/common.yml, docker-compose/kafka_cluster.yml, docker-compose/elastic_cluster.yml, docker-compose/redis_cluster.yml, docker-compose/monitoring.yml, docker-compose/zipkin.yml, docker-compose/services.yml, docker-compose/config_server.yml, docker-compose/twitter_to_kafka.yml, docker-compose/keycloak_postgres.yml, docker-compose/keycloak_authorization_server.yml
  mensagem de commit: "T-006 production-hardening: Modernizar e validar a composição Docker"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium
  ...
  marcar_concluidas T-002 T-004 T-006
  return 0
}
...
Sua tarefa (somente ela):
T-003 — "Restaurar Maven Wrapper e isolar criação de imagens"
  critérios/refs: AC-003 (Maven Wrapper completo), AC-004 (Imagens Docker exigem ação explícita)
  arquivos permitidos (e seus testes): .mvn/wrapper/maven-wrapper.properties, .mvn/wrapper/maven-wrapper.jar, pom.xml, README.md
  mensagem de commit: "T-003 production-hardening: Restaurar Maven Wrapper e isolar criação de imagens"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high &&
    rodar_tarefa 'faixa-3' 'T-007' 'Você executa UMA tarefa da feature "production-hardening" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/production-hardening/spec.md, .spec/features/production-hardening/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-007 — "Documentar operação e adicionar CI"
  critérios/refs: AC-011 (Guia de execução completo), AC-012 (CI verifica segurança, contrato e build), AC-004 (Imagens Docker exigem ação explícita)
  arquivos permitidos (e seus testes): README.md, .github/workflows/ci.yml
  mensagem de commit: "T-007 production-hardening: Documentar operação e adicionar CI"
...
Sua tarefa (somente ela):
T-005 — "Proteger Gateway e API reativa com JWT"
  critérios/refs: AC-007 (Gateway exige token nas rotas de negócio), AC-008 (API reativa exige token nas consultas)
  arquivos permitidos (e seus testes): gateway-service/pom.xml, gateway-service/src/main/java/com/microservices/demo/gateway/service/config/WebSecurityConfig.java, config-server-repository/config-client-gateway.yml, reactive-elastic-query-service/pom.xml, reactive-elastic-query-service/src/main/java/com/microservices/demo/reactive/elastic/query/service/config/WebSecurityConfig.java, config-server-repository/config-client-reactive_elastic_query.yml
  mensagem de commit: "T-005 production-hardening: Proteger Gateway e API reativa com JWT"
...
listar() {
  ...
  echo "  faixa-2  onda 1  T-002, T-004, T-006"
  echo "  faixa-3  onda 1  T-003, T-007"
